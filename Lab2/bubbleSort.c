    ...
    */
	...
	//    MPI_Recv( gdzie, ile , jakiego typu, od kogo, z jakim tagiem, MPI_COMM_WORLD, &status);
	    ...
	    if (status.MPI_TAG==END) {
            ...
            //printf(" MAX: %d \n", max);
            if(rank<size-1) MPI_Send( &dummy, 1, MPI_INT, rank+1, END, MPI_COMM_WORLD);
	    } else {
		//sortowanie babelkowe
            ...
            if(max==-1) max = tmp;
                ...
                if(tmp > max){
                    if(rank<size-1) MPI_Send( &max, 1, MPI_INT, rank+1, MSG_TAG, MPI_COMM_WORLD);
                    max = tmp;
                }
                else{
                    if(rank<size-1)
